  ...
  try {
    if (hassID && !node.hassDevices[hassID]) { // discover the device
      ...
      if (hassDevice.type === 'climate') {
        ...
        // popolate topics object with valueId: valueTopic
      ...
      if (payload) {
        ...
        payload.unique_id = `zwave2mqtt_` + payload.name
  ...
  try {
    ...
    switch (type) {
      ...
      case 'alarm':
        ...
        cfg.object_id += '_' + Constants.alarmType(valueId.index)
        break
      ...
      //   break
      case 'battery':
        cfg = copy(hassCfg.sensor_battery)
        break
      case 'time':
