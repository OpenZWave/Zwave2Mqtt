 ...
 */
function onScanComplete (nodes) { // eslint-disable-line no-unused-vars
...
Gateway.prototype.discoverDevice = function (node, hassDevice) {
  const hassID = hassDevice ? hassDevice.type + '_' + hassDevice.object_id : null
  ...
  try {
    if (hassID && !node.hassDevices[hassID]) { // discover the device
      ...
      if (hassDevice.type === 'climate') {
        ...
        if (payload.max_temp && typeof payload.max_temp === 'string') {
          const maxId = node.values[payload.max_temp]
          payload.max_temp = this.mqtt.getTopic(this.valueTopic(node, maxId))
        }

        if (payload.min_temp && typeof payload.min_temp === 'string') {
          const minId = node.values[payload.min_temp]
          payload.min_temp = this.mqtt.getTopic(this.valueTopic(node, minId))
        }
