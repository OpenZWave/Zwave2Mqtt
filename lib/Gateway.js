...
function onValueChanged (valueId, node, changed) {
  ...
  if (valueConf) {
    ...
    if (valueConf.parseSend) {
      var parsedVal = evalFunction(valueConf.sendFunction)
      if (parsedVal != null) {
        tmpVal = parsedVal
      }
    }
  ...
  tmpVal =
    valueId.type === 'list' && this.config.integerList
      ? valueId.values.indexOf(valueId.value)
      : tmpVal
...
function onNodeStatus (node) {
  ...
  if (node.ready) {
    ...
    var values = this.config.values.filter(
      v => (v.enablePoll || v.verifyChanges) && v.device === node.device_id
    )
...
function getNodeName (node) {
  return (
    (node.loc ? node.loc + '-' : '') +
    (node.name ? node.name : NODE_PREFIX + node.node_id)
  )
}
...
function getMappedValuesTemplate (valueId, modeMap, integerList, defaultValue) {
  ...
  return `{{ ${JSON.stringify(
    finalMap
  )}[value_json.value${valueFilter}] | default('${defaultValue}') }}`
}
  ...
  try {
    ...
    if (this.discovered[valueId.value_id]) {
      ...
      if (valueId.type === 'list' && valueId.values.indexOf(payload) < 0) {
        const hassDevice = this.discovered[valueId.value_id]
        if (hassDevice) {
          // for thermostat_fan_mode command class use the fan_mode_map
          if (valueId.class_id === 0x44 && hassDevice.fan_mode_map) {
    ...
    if (valueConf) {
      ...
      if (valueConf.parseReceive) {
        var parsedVal = evalFunction(valueConf.receiveFunction)
        if (parsedVal != null) {
          payload = parsedVal
        }
      }
...
Gateway.prototype.discoverValue = function (node, valueId) {
  ...
  try {
    ...
    // Set a unique id for the component
    payload.unique_id =
      'zwave2mqtt_' + this.zwave.homeHex + '_' + valueId.value_id
