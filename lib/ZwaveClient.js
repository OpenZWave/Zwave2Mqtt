...
const VAR_TYPES = {
  ...
  bitset: (v, valueId) => {
    ...
    if (binaryValue.length < 8) binaryValue = '0'.repeat(8 - binaryValue.length) + binaryValue

    for (const bit in valueId.bitSetIds) {
      valueId.bitSetIds[bit].value = binaryValue.charAt(8 - parseInt(bit)) === '1'
    }
...
function valueAdded (nodeid, comclass, valueId) {
  ...
  if (!ozwnode) {
    ...
    if (comclass === 0x86 && valueId.index === 2) { // application version
...
ZwaveClient.prototype.startInclusion = function (secure) {
  if (this.client && !this.closed) {
    ...
    this.commandsTimeout = setTimeout(this.stopControllerCommand.bind(this), (this.cfg.commandsTimeout * 1000) || 30000)
...
ZwaveClient.prototype.callApi = async function (apiName, ...args) {
  ...
  if (this.connected) {
    try {
      ...
      var curtomApis = ['_setNodeName', '_setNodeLocation', 'refreshNeighborns', 'getNodes', 'getInfo', '_createScene', '_removeScene', '_setScenes', '_getScenes', '_sceneGetValues', '_addSceneValue', '_removeSceneValue', '_activateScene', 'startInclusion', 'startExclusion', 'stopControllerCommand']
...
ZwaveClient.prototype.writeValue = function (valueId, value) {
  if (this.connected) {
    try {
      ...
      debug(`Error while writing ${value} on ${valueId.value_id}: ${error.message}`)
    }
  }
}
